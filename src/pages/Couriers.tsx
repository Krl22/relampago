...
export default function Couriers() {
  ...
  const handleCreateCourier = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (profileError) {
        ...
        const { data: checkProfile } = await supabase.from('profiles').select('*').eq('id', newUserId).single()
        
        if (!checkProfile) {
             throw new Error('Usuario creado, pero falló al asignar el rol de motorizado. Contacta soporte.')
        }
      }
  ...
  const filteredAndSortedCouriers = useMemo(() => {
    ...
    result = result.filter(courier => {
      const matchName = (courier.full_name || '').toLowerCase().includes(filters.name.toLowerCase())
      
      const courierDate = format(new Date(courier.created_at), 'dd/MM/yyyy', { locale: es })
      const matchDate = filters.date === '' || courierDate.includes(filters.date)
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleCreateCourier} className="space-y-4 py-4">
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">
            Utiliza los filtros en la cabecera de la tabla para buscar motorizados específicos.
          </p>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div className="text-center py-4">Cargando motorizados...</div>
          ) : (
                  ...
                  {filteredAndSortedCouriers.length === 0 ? (
                    ...
                    filteredAndSortedCouriers.map((courier) => (
                      <TableRow key={courier.id}>
                        <TableCell className="font-medium">{courier.full_name || 'Sin nombre'}</TableCell>
                        <TableCell>
