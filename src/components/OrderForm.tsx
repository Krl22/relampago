...
export function OrderForm({ onSuccess }: OrderFormProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      setServerError(error.message || 'Error al crear el pedido. Inténtalo de nuevo.')
    }
  }
      ...
      <CardContent className="pt-6">
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="space-y-2 relative">
            <Label htmlFor="sender_name">Nombre de su negocio (Remitente)</Label>
            <Input 
              id="sender_name" 
            ...
            {errors.sender_name && <span className="text-sm text-destructive">{errors.sender_name.message}</span>}
          </div>

          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              <Label htmlFor="recipient_name">Nombre del destinatario</Label>
              <Input id="recipient_name" {...register("recipient_name")} placeholder="Ej. Juan Pérez" />
              {errors.recipient_name && <span className="text-sm text-destructive">{errors.recipient_name.message}</span>}
            </div>

            <div className="space-y-2">
              <Label htmlFor="recipient_phone">Teléfono del destinatario</Label>
              <Input id="recipient_phone" {...register("recipient_phone")} placeholder="Ej. 999 999 999" type="tel" />
              {errors.recipient_phone && <span className="text-sm text-destructive">{errors.recipient_phone.message}</span>}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {errors.district && <span className="text-sm text-destructive">{errors.district.message}</span>}
          </div>

          <div className="space-y-2">
            <Label htmlFor="delivery_address">Dirección del destinatario (con referencias)</Label>
            <Textarea 
              id="delivery_address" 
              {...register("delivery_address")} 
              placeholder="Ej. Av. Larco 123, frente al parque..." 
            />
            {errors.delivery_address && <span className="text-sm text-destructive">{errors.delivery_address.message}</span>}
          </div>
          ...
          <div className="space-y-2">
            ...
            />
            {errors.product_details && <span className="text-sm text-destructive">{errors.product_details.message}</span>}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              />
              {errors.packages_count && <span className="text-sm text-destructive">{errors.packages_count.message}</span>}
            </div>
            ...
            <div className="space-y-2">
              ...
              />
              <p className="text-xs text-muted-foreground">Si no hay cobro, dejar en 0.</p>
              {errors.amount_to_collect && <span className="text-sm text-destructive">{errors.amount_to_collect.message}</span>}
            </div>
          </div>
